...
8. **Cursor platform rewrites SSH URLs to HTTPS** — The platform injects `insteadOf` rules in `~/.gitconfig` that rewrite `git@github.com:` to HTTPS with its token. This breaks SSH-based cloning of private repos. The update script removes these SSH insteadOf rules on startup so SSH key auth works. The HTTPS rule for `https://github.com/` is kept for this orchestrator repo itself.
9. **SSH key PEM formatting** — The `GITHUB_SSH_PRIVATE_KEY` secret may arrive as a single line. The update script reformats it into valid multi-line PEM before use.
